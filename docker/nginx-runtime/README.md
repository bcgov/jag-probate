# Nginx Runtime for JAG Probate

This directory contains the nginx runtime configuration for deploying the JAG Probate web application in production (OpenShift).
...
```bash
cd docker
docker build -t jag-probate-web-runtime -f nginx-runtime/Dockerfile nginx-runtime/
```
...
spec:
  template:
    spec:
      containers:
        - name: web
          image: jag-probate-web-runtime:latest
          ports:
            - containerPort: 8080
          env:
            - name: API_URL
