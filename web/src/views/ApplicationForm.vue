<template>
  <div id="new-application" class="card bg-white border-white">
    <div class="card home-content border-white">
      ...
      <div v-if="loadingResume" class="form-area d-flex align-items-center justify-content-center">
        <span class="text-muted">Loading application…</span>
      </div>

      <!-- CHEFS Form -->
      <div v-else class="form-area">
        <ChefsFormViewer form-key="legal" :submission-id="chefsSubmissionId" :db-id="dbId" :read-only="isReadOnly"
          @submitted="onSubmitted" @saved="onSaved" @form-error="onFormError" />
      </div>
    </div>
  </div>
</template>
