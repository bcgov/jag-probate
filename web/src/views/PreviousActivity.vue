        ...
        <div v-else class="card bg-white border-white table-section">
          <div class="mx-0 b-table-sticky-header table-responsive-sm" style="max-height: 600px; overflow-y: auto">
            <table role="table" class="table b-table table-striped table-borderless table-sm">
              <thead role="rowgroup">
                <tr role="row">
                  <th style="font-size: 11pt; width: 20%; cursor: pointer" @click="setSort('deceased_name')">
                    <SortIcon column="deceased_name" :sort-key="sortKey" :sort-desc="sortDesc" />Application
                  </th>
                  <th style="font-size: 11pt; width: 20%; cursor: pointer" @click="setSort('lastUpdated')">
                    <SortIcon column="lastUpdated" :sort-key="sortKey" :sort-desc="sortDesc" />Last Updated
                  </th>
                  <th style="font-size: 11pt; width: 20%; cursor: pointer" @click="setSort('lastFiled')">
                    <SortIcon column="lastFiled" :sort-key="sortKey" :sort-desc="sortDesc" />Last Filed
                  </th>
                  <th style="font-size: 10pt; width: 15%; cursor: pointer" @click="setSort('status')">
                    <SortIcon column="status" :sort-key="sortKey" :sort-desc="sortDesc" />Status
                  </th>
              ...
              <tbody role="rowgroup">
                <tr v-for="app in sortedApplications" :key="app.publicId" role="row">
                  <td class="border-top">
                  ...
                  <td class="border-top">
                    ...
                    <button v-if="app.lastFiled === 0" title="Remove Application" type="button"
                      class="btn my-0 py-0 border-0 btn-transparent btn-sm" @click="removeApplication(app)">
                      <svg viewBox="0 0 16 16" width="1.25em" height="1.25em" fill="currentColor" class="text-danger"
                    ...
                    <button title="Resume Application" type="button"
                      class="btn my-0 py-0 border-0 btn-transparent btn-sm" @click="resumeApplication(app)">
                      <svg viewBox="0 0 16 16" width="1.25em" height="1.25em" fill="currentColor" class="text-primary"
                        xmlns="http://www.w3.org/2000/svg">
                        <path
                    ...
                    <button v-if="app.lastFiled !== 0" title="Navigate To Submitted Application" type="button"
                      class="btn my-0 py-0 border-0 btn-transparent btn-sm" @click="navigateToEFilingHub(app.publicId)">
    ...
    <div name="button-menu" class="card button-content border-white bg-white">
      <div class="card-body">
        <p class="process-map-intro-text">
          <strong>The Process Map provides a general overview for a Representation
            Grant.</strong>
        </p>
        <div class="d-flex align-items-center justify-content-between">
          <button type="button" class="btn btn-success process-map-button" @click="showProcessMap = true"
            aria-haspopup="dialog">
            View/Print Process Map
            <span class="fa fa-print ms-1" aria-hidden="true"></span>
          </button>
          <div class="d-flex align-items-center gap-3">
            <button type="button" class="btn btn-success application-button" @click="createCase">
              Begin NEW Session
            </button>
          </div>
        </div>
      ...
      <div class="modal-dialog" role="document">
        <div class="modal-content">
          ...
          <div class="modal-body">
            <div v-if="deleteError" class="mb-3">
              <span class="badge bg-danger text-white p-2 delete-error-badge" :title="deleteErrorMsgDesc">
                {{ deleteErrorMsg }}
                <button type="button" class="btn-close btn-close-white ms-3" @click="deleteError = false" />
              </span>
            </div>
            <h4>
          ...
          <div class="modal-footer">
            <button type="button" class="btn btn-danger" @click="confirmRemoveApplication">
              Confirm
            </button>
            <button type="button" class="btn btn-primary" @click="confirmDelete = false">
              Cancel
            </button>
          </div>
        </div>
