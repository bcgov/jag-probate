<template>
  <div class="wizard-preview-layout">
    ...
    <div class="content-col">
      <!-- CHEFS form viewer-->
      <StepFormViewer ref="stepFormViewerRef" :active-step="activeStepKey" :step-keys="allStepKeys"
        :survey-step-key="surveyStepKey" :substep-to-step-map="substepToStepMap"
        :submission-public-id="submissionPublicId" @survey-complete="onSurveyComplete" @saved="onStepSaved"
        @needs-submission="onNeedsSubmission" @resume-wizard-state="onResumeWizardState"
        @hydration-complete="onHydrationComplete" @step-ready="onStepReady" />
      ...
      <div v-show="showDevControls" class="dev-panel mt-3">
        ...
        <div class="controls mt-4">
          ...
          <div class="d-flex flex-wrap gap-2">
            ...
            <button class="btn btn-sm btn-outline-success" @click="markCurrentComplete">
              Mark current complete
            </button>
            <button class="btn btn-sm btn-outline-danger" @click="markCurrentError">
              Mark current error
            </button>
          </div>

          <!-- Step 1 specific controls -->
          <div v-if="activeStep === firstWizardStepKey" class="mt-3 d-flex flex-wrap gap-2">
      ...
      <!-- Step Nav buttons -->
      <StepNavButtons v-if="!isSurveyStep && isActiveStepLoaded" :has-prev="sidebarRef?.hasPrev ?? false"
        :has-next="sidebarRef?.hasNext ?? false" :is-last-step="sidebarRef?.isLastStep ?? false"
        @prev="sidebarRef?.navigatePrevious()" @next="sidebarRef?.attemptNext()" @submit="onSubmit" />
    </div>
  </div>

  <!-- ── Mobile drawer overlay ───────────────────────────────────────────── -->
  <Teleport to="body">
    <div v-show="layoutStore.mobileNavOpen" class="mobile-drawer-overlay" @click.self="layoutStore.closeMobileNav()">
      <div class="mobile-drawer">
        <div class="mobile-drawer-header">
          ...
          <button class="btn btn-link text-white p-0" aria-label="Close navigation menu"
        ...
        <div v-if="!isSurveyStep && isActiveStepLoaded" class="mobile-drawer-steps">
          <ApplicationStepSidebar :steps="wizardSteps" :initial-step="activeStep" :survey-step-key="surveyStepKey"
            :initial-nav-state="sidebarInitialNavState" :initial-status-map="sidebarInitialStatusMap"
            :initial-disabled-map="sidebarInitialDisabledMap" :register-bridge="false" @navigate="onMobileNavigate" />
        </div>

        <!-- User actions -->
        <div class="mobile-drawer-user">
          <button class="mobile-drawer-action" @click="handlePreviousApplications">
