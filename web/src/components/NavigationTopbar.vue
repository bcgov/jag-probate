<template>
  <header class="app-header">
    <nav class="navbar navbar-expand-lg navbar-dark bg-primary">
      <div class="container-fluid">
        <a class="navbar-brand" href="/">
          <img class="img-fluid d-none d-lg-block" src="/images/bcid-logo-rev-en.svg" width="177" height="44"
            alt="B.C. Government Logo" />
          <img class="img-fluid d-lg-none" src="/images/bcid-symbol-rev.svg" width="63" height="44"
            alt="B.C. Government Logo" />
        </a>
        ...
        <button v-if="authStore.isAuthenticated && isApplicationInProgress"
          class="btn btn-link text-white text-decoration-none btn-save-exit d-lg-none me-3" aria-label="Save and exit"
          :disabled="isSavingAndExiting" @click="handleSaveAndExit">
          <font-awesome-icon icon="floppy-disk" class="me-1" />
          Save and Exit
        </button>

        <!-- Hamburger: always visible on mobile when authenticated -->
        <button v-if="authStore.isAuthenticated" class="btn btn-link text-white p-0 d-lg-none"
          aria-label="Open navigation menu" @click="layoutStore.openMobileNav()">
          <font-awesome-icon icon="bars" size="lg" />
        </button>

        <!-- Desktop user dropdown -->
        <div v-if="authStore.isAuthenticated" class="navbar-nav ms-auto d-none d-lg-flex align-items-center">
          <button v-if="isApplicationInProgress" class="btn btn-link text-white text-decoration-none btn-save-exit me-3"
            :disabled="isSavingAndExiting" @click="handleSaveAndExit">
            <font-awesome-icon icon="floppy-disk" class="me-1" />
            Save and Exit
          </button>

          <BDropdown variant="link" no-caret menu-class="dropdown-menu-end"
            ...
            <BDropdownItem v-if="!isOnPreviousActivity" @click="handlePreviousApplications">
              <font-awesome-icon icon="fas-solid fa-list" class="me-2" />
              Previous Applications
            </BDropdownItem>
            <BDropdownItem @click="handleLogout">
              <font-awesome-icon icon="fas-solid fa-right-from-bracket" class="me-2" />
              Log out
            </BDropdownItem>
          </BDropdown>
        </div>
    ...
    <div v-if="saveAndExitError" class="alert alert-danger m-0 rounded-0 text-center" role="alert">
      We could not save your application. Please try again.
    </div>

    <!-- Mobile drawer for non-wizard pages (wizard pages use their own drawer) -->
    <Teleport to="body">
      <div v-if="authStore.isAuthenticated && !layoutStore.hasMobileNav" v-show="layoutStore.mobileNavOpen"
        class="topbar-mobile-overlay" @click.self="layoutStore.closeMobileNav()">
        <div class="topbar-mobile-drawer">
          <div class="topbar-mobile-drawer-header">
            ...
            <button class="btn btn-link text-white p-0" aria-label="Close menu" @click="layoutStore.closeMobileNav()">
              <font-awesome-icon icon="xmark" size="lg" />
            </button>
          </div>
          <div class="topbar-mobile-drawer-actions">
            <button v-if="isApplicationInProgress" class="topbar-mobile-action" :disabled="isSavingAndExiting"
              @click="handleSaveAndExit">
              <font-awesome-icon icon="floppy-disk" class="me-2" />
              Save and Exit
            </button>
            <button v-if="!isOnPreviousActivity" class="topbar-mobile-action" @click="handlePreviousApplications">
