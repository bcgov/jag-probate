<template>
  ...
  >
    <template #target="{ visible }">
      <button
        type="button"
        class="btn-link-glossary text-primary fw-semibold p-0"
      ...
      >
        {{ term }}
        <font-awesome-icon
          icon="fas-solid fa-circle-question"
          class="text-primary"
          aria-hidden="true"
        />
      </button>
...
<script setup lang="ts">
  ...
  const termDefinition = computed(
    () => glossary[props.term.toLowerCase()] || 'Definition not found.'
  );
</script>
<style scoped>
