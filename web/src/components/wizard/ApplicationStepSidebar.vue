<template>
  <div class="wiz-sidebar" :class="{ 'is-collapsed': collapsed }">
    <div class="wiz-header">
      ...
      <button class="wiz-toggle-btn" :aria-label="collapsed ? 'Expand sidebar' : 'Collapse sidebar'"
        :title="collapsed ? 'Expand sidebar' : 'Collapse sidebar'" @click="collapsed = !collapsed">
    ...
    <div class="wiz-steps-container">
      ...
      <button class="wiz-survey-btn" type="button" aria-label="Back to Survey"
        :title="collapsed ? 'Back to Survey' : undefined" @click="goToSurvey">
      ...
      <template v-for="step in steps" :key="step.key">
        <!-- Step header row -->
        <div class="wiz-step" :class="{
          'is-active': isStepActive(step.key),
          'is-disabled': isStepDisabled(step.key),
          'is-ghost': isStepHidden(step.key),
        }" @click="onStepClick(step)" @mouseenter="onStepMouseEnter(step, $event)" @mouseleave="onStepMouseLeave">
          <div class="wiz-icon" :class="stepIconClass(step)">
            <font-awesome-icon v-if="stepIconName(step)" :icon="stepIconName(step)!" />
          ...
          <button v-if="!isStepActive(step.key)" class="wiz-expand-btn" :aria-label="expandedSteps[step.key] ? 'Collapse substeps' : 'Expand substeps'
            " @click.stop="toggleExpand(step.key)">
            <font-awesome-icon icon="chevron-down" :class="{ 'is-rotated': isSubMenuOpen(step.key) }" />
          </button>
        </div>

        <!-- Substep list -->
        <div class="wiz-subitems" :style="{ display: isSubMenuOpen(step.key) ? 'block' : 'none' }">
          <div v-for="substep in step.substeps" :key="substep.key" class="wiz-subitem" :class="{
  ...
  <Teleport to="body">
    <div v-if="collapsed && hoveredStepData" class="wiz-flyout"
      :style="{ top: flyoutTop + 'px', left: flyoutLeft + 'px' }" @mouseenter="cancelHoverClear"
      @mouseleave="clearHover">
      <div class="wiz-flyout-title">{{ hoveredStepData.title }}</div>
      <div v-for="substep in hoveredStepData.substeps" :key="substep.key" class="wiz-flyout-item" :class="{
        'is-active': activeSubstep === substep.key,
        'is-disabled': isSubstepDisabled(substep.key),
        'is-ghost': isSubstepHidden(substep.key),
      }" @click="
