...
public static class SubmissionEnricher
{
    ...
    private static void CollectFromSpouseData(
        JObject root, string deceasedName,
        List<JObject> minors, List<JObject> incapableAdults)
    {
        ...
        for (int i = 1; i < spouseData.Count; i++)
        {
            ...
            if (s == null) continue;
            if (s.Value<string>("spouseIsAlive") != "yes") continue;

            if (s.Value<string>("spouseIsAdult") == "no")
            {
                minors.Add(BuildMinor(s, "spouse", deceasedName, "spouse"));
            }
            else if (s.Value<string>("spouseIsAdult") == "yes"
        ...
        List<JObject> minors, List<JObject> incapableAdults)
    {
        ...
        for (int i = 1; i < childData.Count; i++)
        {
            ...
            if (c == null) continue;
            if (c.Value<string>("childIsAlive") != "yes") continue;

            if (c.Value<string>("childIsAdult") == "no")
            {
                minors.Add(BuildMinor(c, "child", deceasedName, "child"));
            }
            else if (c.Value<string>("childIsAdult") == "yes"
    ...
    private static JObject BuildMinor(JObject src, string prefix, string deceasedName, string role)
    {
        ...
        var postalAddress = src.Value<string>($"{p}HasDiffMail") == "yes"
            ? FormatAddress(
        ...
        var guardianPostalAddress = src.Value<string>($"{g}HasDiffMail") == "yes"
            ? FormatAddress(
        ...
        return new JObject
        {
            ...
            ["minorEmail"] = NoneIfEmpty(src.Value<string>($"{p}Email"), src.Value<string>($"{p}HasEmail")),
            ["minorFax"] = NoneIfEmpty(src.Value<string>($"{p}Fax"), src.Value<string>($"{p}HasFax")),
            ["minorGuardianName"] = src.Value<string>($"{prefix}HasGuardian") == "yes"
                ? (src.Value<string>($"{g}Name") ?? "None")
                : "None",
            ["minorGuardianResAddress"] = guardianResAddress,
            ["minorGuardianPostalAddress"] = guardianPostalAddress,
            ["minorGuardianEmail"] = NoneIfEmpty(src.Value<string>($"{g}Email"), src.Value<string>($"{g}HasEmail")),
            ["minorGuardianFax"] = NoneIfEmpty(src.Value<string>($"{g}Fax"), src.Value<string>($"{g}HasFax")),
        };
    }

    private static JObject BuildIncapableAdult(JObject src, string prefix, string deceasedName, string role)
    {
        var name = src.Value<string>($"{prefix}Name") ?? "";
        var p = $"{prefix}Incomp";
        ...
        var nomineePostalAddress = src.Value<string>($"{n}HasDiffMail") == "yes"
            ? FormatAddress(
        ...
        return new JObject
        {
            ...
            ["incapableAdultEmail"] = NoneIfEmpty(src.Value<string>($"{p}Email"), src.Value<string>($"{p}HasEmail")),
            ["incapableAdultFax"] = NoneIfEmpty(src.Value<string>($"{p}Fax"), src.Value<string>($"{p}HasFax")),
            ["incapableAdultNomineeName"] = src.Value<string>($"{prefix}HasNominee") == "yes"
            ...
            ["incapableAdultNomineeEmail"] = NoneIfEmpty(src.Value<string>($"{n}Email"), src.Value<string>($"{n}HasEmail")),
            ["incapableAdultNomineeFax"] = NoneIfEmpty(src.Value<string>($"{n}Fax"), src.Value<string>($"{n}HasFax")),
        };
    }
    ...
    public static object EnrichP9(object submissionData)
    {
        ...
        if (notifyData != null)
        {
            foreach (var item in notifyData.OfType<JObject>())
            {
                if (item.Value<string>("p1Delivered") != "yes") continue;

                var name = item.Value<string>("recipientName") ?? "";
                var role = item.Value<string>("recipientRole") ?? "";
                var displayName = !string.IsNullOrWhiteSpace(role)
                    ? $"{name} ({role})"
                    : name;
    ...
    private static string FormatAddress(string? street, string? city, string? province, string? postal, string? country)
    {
    ...
    {
        if (string.IsNullOrWhiteSpace(isoDate)) return "None";
        if (DateTimeOffset.TryParse(isoDate, CultureInfo.InvariantCulture, DateTimeStyles.None, out var dt))
